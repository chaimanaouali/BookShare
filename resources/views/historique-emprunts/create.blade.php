...
@section('title', 'Créer une Entrée d\'Historique')
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0">Créer une Nouvelle Entrée d'Historique</h5>
                </div>
                <div class="card-body">
                    <form action="{{ route('historique-emprunts.store') }}" method="POST">
                        @csrf
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('emprunt_id') is-invalid @enderror" id="emprunt_id" name="emprunt_id" required>
                                    ...
                                    @foreach($emprunts as $emprunt)
                                        <option value="{{ $emprunt->id }}" {{ old('emprunt_id') == $emprunt->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('action') is-invalid @enderror" id="action" name="action" required>
                                    ...
                                    <option value="Création" {{ old('action') == 'Création' ? 'selected' : '' }}>Création</option>
                                    <option value="Modification" {{ old('action') == 'Modification' ? 'selected' : '' }}>Modification</option>
                                    <option value="Suppression" {{ old('action') == 'Suppression' ? 'selected' : '' }}>Suppression</option>
                                    <option value="Retour" {{ old('action') == 'Retour' ? 'selected' : '' }}>Retour</option>
                                    <option value="Prolongation" {{ old('action') == 'Prolongation' ? 'selected' : '' }}>Prolongation</option>
                                    <option value="Pénalité" {{ old('action') == 'Pénalité' ? 'selected' : '' }}>Pénalité</option>
                                </select>
                                @error('action')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="datetime-local" class="form-control @error('date_action') is-invalid @enderror" 
                                       ...
                                       value="{{ old('date_action', now()->format('Y-m-d\TH:i')) }}" required>
                                @error('date_action')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-12 mb-3">
                                ...
                                <textarea class="form-control @error('details') is-invalid @enderror" 
                                          ...
                                          placeholder="Décrivez les détails de l'action...">{{ old('details') }}</textarea>
                                @error('details')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
